  ...
  return (
    <section className="bg-gray-50 font-inter py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        <div className="bg-white rounded-xl p-6 shadow-sm flex justify-center items-center border aspect-square lg:max-w-md mx-auto">
          <img
            src={product.image}
            alt={product.name}
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="mt-6">
            ...
            {product.originalPrice && (
              <span className="ml-3 text-lg text-gray-400 line-through">Rs. {product.originalPrice}</span>
            )}
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center border rounded-lg overflow-hidden">
              <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="px-4 py-2 text-gray-600 hover:bg-gray-100 transition" data-testid="quantity-decrease-button">
                <Minus size={16} />
              </button>
              <span className="px-5 font-bold text-md text-gray-800" data-testid="quantity-display">{quantity}</span>
              <button onClick={() => setQuantity(q => q + 1)} className="px-4 py-2 text-gray-600 hover:bg-gray-100 transition" data-testid="quantity-increase-button">
                <Plus size={16} />
              </button>
            </div>
          </div>
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Button onClick={handleBuyNow} disabled={product.countInStock === 0} testId="buy-now-button">
              Buy Now
            </Button>
            <Button onClick={() => addToCart(product, quantity)} disabled={product.countInStock === 0} variant="outline" icon={<ShoppingCart size={18} />} testId="add-to-cart-button">
              Add to Cart
            </Button>
          </div>
          <div className="mt-4">
